...
export const useOrganizationStore = create((set) => ({
  ...
  createEventOndb: async function (eventData) {
    try {
      ...
      const eventId = res.data.event?._id || res.data.eventId || res.data._id;
  ...
  getInventoryById: async (inventoryId) => {
    try {
      ...
      console.log("error in fetching inventory by id for dashboard : ", err);
      if (err.response) {
  ...
  getEventDetailsById: async (eventId) => {
    try {
      const res = await axiosInstance.get(
        `/organize/organization/get-event/${eventId}`
      );
  ...
  getInventoryByEventId: async (eventId) => {
    try {
      ...
      console.log("error in fetching inventory by event id : ", err);
      if (err.response) {
        toast.error(err.response.data?.message || "Failed to Fetch Inventory");
      }
    }
  ...
  updateEvent: async (eventId, eventData) => {
    try {
      const res = await axiosInstance.put(
        `/organize/organization/update-event/${eventId}`,
        eventData
      );
      if (res.data.success) {
  ...
  updateInventory: async (eventId, inventoryData) => {
    try {
      const res = await axiosInstance.put(
        `/booking/inventory/update-inventory/${eventId}`,
        inventoryData
      );
      if (res.data.success) {
